...
function formatTimeAgo(date) {
  ...
  <h2 className="text-4xl md:text-5xl font-extrabold text-white text-center mb-10 tracking-tight leading-tigh mt-5">
   Users Blogs
</h2>

{
  loading ? (
    <div className="text-center py-20">
      <p className="text-zinc-400 mt-2">loadidng blogs...</p>
    </div>
  ) : (

    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
  {blogs.filter((e) => e.writer === user.name ).map((blog, index) => (
    <a
